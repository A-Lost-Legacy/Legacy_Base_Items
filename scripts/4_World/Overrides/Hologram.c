modded class Hologram {
	override void UpdateHologram( float timeslice ) {
        if (GetLegacyBaseStorageConfig().EnabledebugLogging) {
            GetLegacy_Base_StorageLogger().LogDebug("[DEBUG] UpdateHologram called: timeslice=" + timeslice);
        }
		...
		if(container) {
            ...
            if (GetLegacyBaseStorageConfig().EnabledebugLogging) {
                GetLegacy_Base_StorageLogger().LogDebug("[DEBUG] UpdateHologram: Custom projection for container.");
            }
			return;
		}
	}
	...
	override string GetProjectionName(ItemBase item) {
        if (GetLegacyBaseStorageConfig().EnabledebugLogging) {
            GetLegacy_Base_StorageLogger().LogDebug("[DEBUG] GetProjectionName called: item=" + item);
        }
		Legacy_Kit item_in_hands = Legacy_Kit.Cast( item );
		if ( item_in_hands ) {
            if (GetLegacyBaseStorageConfig().EnabledebugLogging) {
                GetLegacy_Base_StorageLogger().LogDebug("[DEBUG] GetProjectionName: Legacy_Kit item, returning custom name.");
            }
	...
	override void EvaluateCollision(ItemBase action_item = null) {
        if (GetLegacyBaseStorageConfig().EnabledebugLogging) {
            GetLegacy_Base_StorageLogger().LogDebug("[DEBUG] EvaluateCollision called: action_item=" + action_item);
        }			
		ItemBase item_in_hands = m_Parent;
		if (item_in_hands.IsInherited(Legacy_Kit) || item_in_hands.IsInherited(Legacy_Storage_Base)) {
            if (GetLegacyBaseStorageConfig().EnabledebugLogging) {
                GetLegacy_Base_StorageLogger().LogDebug("[DEBUG] EvaluateCollision: Skipping collision for Legacy_Kit/Storage.");
            }
	...
	override void RefreshVisual() {
        if (GetLegacyBaseStorageConfig().EnabledebugLogging) {
            GetLegacy_Base_StorageLogger().LogDebug("[DEBUG] RefreshVisual called");
        }
		if ( m_Projection && m_Projection.IsKindOf("Legacy_Storage_Base") ) {
			...
			string textureName = "";
			textureName = string.Format("#(argb,8,8,3)color(%1,%2,%3,0.5,ca)",GetLegacyBaseStorageConfig().HologramColor_R,GetLegacyBaseStorageConfig().HologramColor_G,GetLegacyBaseStorageConfig().HologramColor_B);
            ...
            if (GetLegacyBaseStorageConfig().EnabledebugLogging) {
                GetLegacy_Base_StorageLogger().LogDebug("[DEBUG] RefreshVisual: Refreshed Legacy_Storage_Base hologram materials");
            }
		} else {
			super.RefreshVisual();
		}
	...
	override bool IsFloating() {
        if (GetLegacyBaseStorageConfig().EnabledebugLogging) {
            GetLegacy_Base_StorageLogger().LogDebug("[DEBUG] IsFloating called");
        }
		if (m_Parent.IsInherited(Legacy_Kit) || m_Parent.IsInherited(Legacy_Storage_Base) ) {
            if (GetLegacyBaseStorageConfig().EnabledebugLogging) {
                GetLegacy_Base_StorageLogger().LogDebug("[DEBUG] IsFloating: true for Legacy_Kit or Legacy_Storage_Base");
            }
			return true;
		}
	...
	override void SetProjectionPosition( vector position ) {
        if (GetLegacyBaseStorageConfig().EnabledebugLogging) {
            GetLegacy_Base_StorageLogger().LogDebug("[DEBUG] SetProjectionPosition called: position=" + position);
        }	
		if (m_Parent.IsInherited(Legacy_Kit) || m_Parent.IsInherited(Legacy_Storage_Base) && IsFloating()) { 
			vector itemPos = SetOnGroundOld( position ) + m_Parent.Get_ItemPlacingPos();
			m_Projection.SetPosition( itemPos );
            if (GetLegacyBaseStorageConfig().EnabledebugLogging) {
                GetLegacy_Base_StorageLogger().LogDebug("[DEBUG] SetProjectionPosition: Custom projection for Legacy_Kit/Storage_Base");
            }
			return;
		}
	...
	vector SetOnGroundOld( vector position ) {
        if (GetLegacyBaseStorageConfig().EnabledebugLogging) {
            GetLegacy_Base_StorageLogger().LogDebug("[DEBUG] SetOnGroundOld called: position=" + position);
        }
	...
	override protected vector GetProjectionEntityPosition( PlayerBase player ) {
        if (GetLegacyBaseStorageConfig().EnabledebugLogging) {
            GetLegacy_Base_StorageLogger().LogDebug("[DEBUG] GetProjectionEntityPosition called: player=" + player);
        }
